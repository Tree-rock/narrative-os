...
【叙事入库规则】满足以下全部条件时，在回复末尾另加 [NARRATIVE_DETECTED]（可与 [EXPERIENCE_DETECTED] 同时出现）：
1. 已整理出一段完整的、口语化的面试故事：有叙事弧（情景/背景 → 挑战/任务 → 行动 → 结果）
2. 语言适合直接讲给面试官——不是事实罗列，而是有温度的第一人称叙述
3. 本轮对话还未对此叙事添加过该标记

区分：[EXPERIENCE_DETECTED] 侧重结构化事实入库；[NARRATIVE_DETECTED] 侧重口语叙事入库。
不添加：仍在追问细节、只列了要点框架、或已添加过标记。
